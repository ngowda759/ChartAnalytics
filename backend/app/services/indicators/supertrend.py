...
def calculate_supertrend(
    ...
    for i in range(1, len(closes)):
        ...
        if curr_trend == "up":
            curr_st = max(lower_band[i], supertrend[i - 1] if supertrend[i - 1] else lower_band[i])
        else:
            curr_st = min(upper_band[i], supertrend[i - 1] if supertrend[i - 1] else upper_band[i])
        
        reversal = (prev_trend == "down" and curr_trend == "up") or (prev_trend == "up" and curr_trend == "down")
