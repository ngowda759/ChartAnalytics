...
def interpret_rsi(
    rsi_value: float, oversold: float = 30, overbought: float = 70
) -> RSISignal:
    """
    ...
    if rsi_value <= oversold:
        ...
        return RSISignal(
            value=rsi_value, signal="oversold", strength=min(100, strength)
        )
    elif rsi_value >= overbought:
        # How far into overbought territory
        strength = ((rsi_value - overbought) / (100 - overbought)) * 100
        return RSISignal(
            value=rsi_value, signal="overbought", strength=min(100, strength)
        )
    else:
        # Neutral zone
        if rsi_value < 50:
