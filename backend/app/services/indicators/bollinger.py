...
def detect_bollinger_squeeze(
    ...
    for i in range(lookback - 1, len(bands)):
        ...
        recent_bands = [b.bandwidth for b in bands[i - lookback + 1:i + 1] if b is not None]
...
def calculate_bollinger_width(bands: List[Optional[BollingerBands]]) -> List[Optional[float]]:
    """
...
def calculate_bollinger_percent_b(bands: List[Optional[BollingerBands]]) -> List[Optional[float]]:
    """
