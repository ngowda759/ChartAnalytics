...
from app.services.indicators import (
    ...
    calculate_bollinger_bands, calculate_atr, detect_ema_crossover,
    interpret_rsi, get_trend_direction
)
...
class AIMarketEngine:
    ...
    def analyze(
        ...
        signals = self._generate_signals(
            closes, ema_20, ema_50, ema_200, rsi_value, macd_data, adx_data, bb_data, volumes
        )
    ...
    def _calculate_adx(self, highs: List[float], lows: List[float], closes: List[float]) -> List:
        """Calculate ADX."""
    ...
    def _determine_trend(
        ...
        trend_dir = get_trend_direction(ema_20[-1], ema_50[-1], ema_200[-1], current_price)
    ...
    def _generate_observations(
        ...
        if len(prices) >= 5:
            ...
            if abs(recent_change) > 2:
                observations.append(f"Significant {recent_change:.1f}% move in last 5 periods")
    ...
    def _generate_summary(
        ...
        if support and resistance:
            summary_parts.append(f"Trading between {support[0].price:.0f} and {resistance[0].price:.0f}")
