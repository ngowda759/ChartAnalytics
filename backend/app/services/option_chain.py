...
class OptionChainAnalyzer:
    ...
    def _process_chain_data(self, chain_data: List[OptionChainData]) -> List[StrikeAnalysis]:
        ...
        for data in chain_data:
            ...
            interpretation = self._interpret_strike(
                data.strike, data.call_oi, data.put_oi, 
                data.call_change_oi, data.put_change_oi
            )
    ...
    def _determine_net_building(
        self, call_change_oi: int, put_change_oi: int
    ) -> str:
    ...
    def _find_max_pain(self, strikes: List[StrikeAnalysis]) -> float:
        ...
        for strike_data in strikes:
            ...
            pain[strike] = sum(
                max(0, self.spot_price - s.strike) * s.call_oi 
                for s in strikes
            ) + sum(
                max(0, s.strike - self.spot_price) * s.put_oi
                for s in strikes
            )
    ...
    def _analyze_trend(
        ...
        # OI distribution
        atm_idx = next((i for i, s in enumerate(strikes) 
                       if s.strike == self._find_atm_strike(strikes)), 0)
        
        upper_oi = sum(s.put_oi for s in strikes[:atm_idx] if s.strike < self.spot_price)
        lower_oi = sum(s.call_oi for s in strikes[atm_idx:] if s.strike >= self.spot_price)
    ...
    def _find_support_levels(self, strikes: List[StrikeAnalysis]) -> List[Dict[str, Any]]:
    ...
    def _find_resistance_levels(self, strikes: List[StrikeAnalysis]) -> List[Dict[str, Any]]:
    ...
    def _generate_interpretation(
        ...
        # PCR interpretation
        if pcr > 1.5:
            interpretations.append(f"PCR of {pcr:.2f} is very high, indicating strong hedging activity")
        elif pcr > 1.2:
            interpretations.append(f"PCR of {pcr:.2f} suggests bullish sentiment")
        elif pcr < 0.7:
            interpretations.append(f"PCR of {pcr:.2f} is low, suggesting bearish sentiment")
        else:
...
def calculate_max_pain(chain_data: List[OptionChainData], spot_price: float) -> float:
    ...
    for data in chain_data:
        pain = sum(
            max(0, spot_price - d.strike) * d.call_oi
            for d in chain_data
        ) + sum(
            max(0, d.strike - spot_price) * d.put_oi
            for d in chain_data
        )
