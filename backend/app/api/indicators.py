...
from app.services.indicators import (
    ...
    calculate_supertrend, interpret_rsi, get_trend_direction
)
...
async def get_indicators(symbol: str):
    ...
    return TechnicalIndicators(
        ...
        ) if st else SupertrendData(value=current_price, trend="up", reversal=False),
        adx=ADXData(
        ...
        ) if adx else ADXData(adx=25, plus_di=15, minus_di=15, trend_strength="moderate"),
        atr=ATRData(
            value=round(atr_value, 2),
            normalized=round(atr_value / current_price * 100, 2) if current_price > 0 else 0,
        ),
    )
...
async def get_indicator_history(
    symbol: str,
    indicator: str = Query(..., description="Indicator name (ema, rsi, macd, adx, bollinger, atr, vwap)"),
    ...
    if indicator == "ema":
        ...
        values = [
            {"upper": round(bb.upper, 2) if bb else None, "middle": round(bb.middle, 2) if bb else None, "lower": round(bb.lower, 2) if bb else None}
            for bb in bb_data
        ]
    elif indicator == "atr":
        ...
        values = [
            {"adx": round(adx.adx, 2) if adx else None, "plus_di": round(adx.plus_di, 2) if adx else None, "minus_di": round(adx.minus_di, 2) if adx else None}
            for adx in adx_data
        ]
    else:
...
async def get_ai_analysis(symbol: str):
    ...
    return {
        ...
        "signals": [
            {"indicator": s.indicator, "value": s.value, "signal": s.signal, "strength": s.strength, "message": s.message}
            for s in insight.signals
        ],
        "support_levels": [{"price": s.price, "strength": s.strength} for s in insight.support_levels],
        "resistance_levels": [{"price": s.price, "strength": s.strength} for s in insight.resistance_levels],
